...
const refreshAuth = async (refreshToken: string): Promise<AuthTokensResponse> => {
    try {
        ...
        const role = refreshTokenData.role;

        const userId = refreshTokenData.consumerId ? refreshTokenData.consumerId : refreshTokenData.brandUserId;
        await prisma.token.delete({ where: { id: refreshTokenData.id } });
        return tokenService.generateAuthTokens({ id: userId! }, role);
    } catch (error) {
